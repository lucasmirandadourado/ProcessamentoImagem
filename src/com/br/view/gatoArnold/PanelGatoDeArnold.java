        ...
        while(flag == false){
        	...
        	for (int i = 0; i < altura; i++) {
    			for (int j = 0; j < altura; j++) {
    				matrizResultado[i][j] = matrizInput[(int)(i + j) % 1][(int)(i + 2*j) % 1];
    				imagemGatoDeArnold.setRGB(j, i, corPixel(matrizResultado[i][j]));
    				validate();
    				repaint();
    			}
        	}
        	...
        	if(equals(matrizResultado, matrizDaImagem1) == true){
        		flag = true;
        		System.out.println("Entrou no IF-----------------------------------------------");
        	} else {
        		flag = false;
        		matrizInput = matrizResultado;
